    ...
    <div class="profile-avatar flex-1 flex">
        ...
        >
            <div class="flex-1 flex flex-col items-center justify-center space-y-4 dz-message">
                <button class="px-10 py-3 rounded-full border-2 border-black uppercase font-bold"><i class="fas fa-upload mr-1"></i>choose a photo</button>
...
export default {
    mixins: [CropperMixin],
    mounted() {
        console.log('SettingsProfileAvatar::mounted');
    },
    props: {
        dropzoneConfig: {
            type: Object,
            default: () => ({
                url: ' ',
                dictDefaultMessage: '<button class="px-10 py-3 rounded-full border-2 border-black uppercase font-bold"><i class="fas fa-upload mr-1"></i>choose a photo</button><p class="italic text-gray text-xs">Max file size: <span class="font-bold">5MB</span></p>',
    ...
    methods: {
        test() {
            console.log('SettingsProfileAvatar::test');
        },
    },
}
</script>
