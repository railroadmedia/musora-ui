<template>
    <a href="#" class="rudiment-card flex flex-row items-center px-3 py-1">
        <div class="w-32 rounded-lg overflow-hidden relative">
            <img :src="video.thumbnail" class="object-cover object-center w-full h-auto">
            <div class="absolute bottom-0 left-0 rudiment-card-teacher-gradient text-white pl-2 pb-1 uppercase text-xs font-bold">{{ video.teacher }}</div>
        ...
        <div class="flex items-center w-full">
            <div class="flex-1 pl-4 capitalize font-bold">{{ video.title }}</div>
            <div class="flex-none h-16 px-8 hidden small:block">
                <img :src="video.sheet" class="h-full w-auto">
            </div>
